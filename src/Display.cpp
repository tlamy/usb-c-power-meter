...
void Display::begin() { u8g2.begin(); }

void Display::drawStrCentered(int line, const char *buf) {
    u8g2.drawStr((u8g2.getDisplayWidth() - u8g2.getStrWidth(buf)) / 2, line, buf);
}

void Display::drawStrRight(int line, const char *buf) {
    u8g2.drawStr(u8g2.getDisplayWidth() - u8g2.getStrWidth(buf), line, buf);
}
