...
void Display::begin() {
    ...
    u8g2->begin();
    this->width = 128; //u8g2->getDisplayWidth();
    this->height = 64; //u8g2->getDisplayHeight();
...
void Display::display_measurements(PowerMeasurement measurement) {
    ...
    do {
        if (volt_norm == 0) {
            ...
            sprintf(buf, "%2.2fV", measurement.voltage);
            ...
            u8g2->drawLine(width - 1, y_bar - 1, width - 1, y_bar + 1);
            int bar = static_cast<int>(static_cast<float>(width) * measurement.current / bar_maxcurrent);
            u8g2->drawLine(0, y_bar, bar, y_bar);
            bar = static_cast<int>(static_cast<float>(width) * display_current / bar_maxcurrent);
            u8g2->drawLine(bar, y_bar - 1, bar, y_bar + 1);

            u8g2->setFont(u8g2_font_profont29_mf);
            sprintf(buf2, "%0.3fA", amps);
            drawStrRight(height - 1, buf2);
        }
