...
#define RELEASE_VERSION "bodger-0.1.0"
...
void display(int millivolt, uint8_t volt_norm, int current, int maxcurrent) {
    char buf[32];
    char buf2[32];
    float watts = (static_cast<float>(millivolt) * static_cast<float>(current)) / static_cast<float>(1000000.0);
    u8g2.firstPage();
    do {
        u8g2.setFont(u8g2_font_profont17_tf);
        if (volt_norm == 0)
            u8g2.drawStr(0, 17, "Off");
        else {
            sprintf(buf, "%0.3fV", ((float) millivolt / 1000.0));
            u8g2.drawStr(0, 17, buf);
            sprintf(buf, "%02.3fW", watts);
            u8g2.drawStr(127 - u8g2.getStrWidth(buf), 17, buf);
            // else if(volt_norm == 5)
