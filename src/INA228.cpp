...
// PURPOSE: Arduino library for the INA228, I2C, 20 bit, voltage, current and
// power sensor.
...
//  CONSTRUCTOR
//
INA228::INA228(const uint8_t address, TwoWire *wire) : _ADCRange(false) {
  _address = address;
  _wire = wire;
  //  no calibrated values by default.
  _shunt = 0.015;
  _maxCurrent = 10.0;
  _current_LSB = _maxCurrent * static_cast<float>(pow(2, -19));
  _error = 0;
}
...
int32_t INA228::getShuntVoltageRAW() {
  int32_t value =
      static_cast<int32_t>(_readRegister(INA228_SHUNT_VOLTAGE, 3)) >> 4;
  //  handle negative values (20 bit)
  if ((value & 0x00080000) > 0) {
    value |= 0xFFF00000; // NOLINT(*-narrowing-conversions)
  } // NOLINT(*-narrowing-conversions)
...
int INA228::setMaxCurrentShunt(float maxCurrent, float shunt) {
  ...
  _current_LSB = _maxCurrent / 524288.0F;

  //  PAGE 31 (8.1.2)
  float shunt_cal = 13107200000.0F * _current_LSB * shunt;
  //  depends on ADCRANGE in INA228_CONFIG register.
  if (_ADCRange) {
    shunt_cal *= 4;
  }
  auto value = static_cast<uint16_t>(shunt_cal);
...
float INA228::getMaxCurrent() const { return _maxCurrent; }

float INA228::getShunt() const { return _shunt; }
...
//  PRIVATE
//
uint32_t INA228::_readRegister(uint8_t reg, uint8_t bytecount) {
  this->_error = 0;
  this->_wire->beginTransmission(_address);
  this->_wire->write(reg);
  int status = this->_wire->endTransmission();
  if (status != 0) {
    this->_error = -1;
    return 0;
  }
  ...
  uint32_t value = 0;
  if (bytecount == this->_wire->requestFrom(this->_address, bytecount)) {
    for (int i = 0; i < bytecount; i++) {
      value <<= 8;
      value |= this->_wire->read() & 0xff;
    }
  } else {
    this->_error = -2;
    return 0;
